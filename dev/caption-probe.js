...
;(() => {
  ...
  if (!region) {
    console.warn(
      "[probe] no captions region — enable captions (press c) then re-run",
    )
    return
  }
  console.log(
    "[probe] region matched:",
    region.getAttribute("aria-label") || region.getAttribute("jsname"),
  )
  ...
  function items() {
    ...
    if (!els.length)
      els = [...region.querySelectorAll(":scope > *")].filter((b) =>
        b.querySelector("img"),
      )
    return els
      .map((el) => {
  ...
  const P = { recs: [], maxConcurrent: 0, t0: Date.now() }
  window.__probe = P
  function snap(reason) {
    ...
    P.recs.push({
      ...
      reason,
      n: its.length,
      items: its.map((i) => ({ s: i.speaker, t: i.text.slice(0, 60) })),
    })
  ...
  P.report = () => {
    ...
    console.log(
      "max concurrent items seen:",
      P.maxConcurrent,
      "(>1 confirms Meet renders one item per active speaker)",
    )
    console.log("ticks with >1 speaker:", multi.length)
    if (multi.length)
      console.log(
        "sample concurrent tick:",
        JSON.stringify(multi[Math.floor(multi.length / 2)], null, 2),
      )
    ...
    for (const r of P.recs)
      for (const it of r.items) {
        ...
        if (
          prev &&
          !it.t.startsWith(prev.slice(0, Math.min(prev.length, 8))) &&
          it.t.length < prev.length
        )
          shrinks.push({
            dt: r.dt,
            speaker: it.s,
            from: prev.slice(0, 40),
            to: it.t.slice(0, 40),
          })
        last[it.s] = it.t
      }
    console.log(
      "text-shrink events (turn boundaries / potential overlap points):",
      shrinks.length,
    )
    if (shrinks.length)
      console.log("samples:", JSON.stringify(shrinks.slice(0, 5), null, 2))
    console.log("raw timeline in __probe.recs (", P.recs.length, "records )")
    return {
      maxConcurrent: P.maxConcurrent,
      multiSpeakerTicks: multi.length,
      shrinkEvents: shrinks.length,
    }
  }

  console.log(
    "[probe] recording. Talk (ideally 2 people at once + a mid-turn pause), then run  __probe.report()",
  )
})()
